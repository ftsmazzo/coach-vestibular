...
export function parseProvaQuestoesCsv(
  ...
  const iArea = idx(["area", "bloco", "area grande", "grupo"]);
  ...
  for (let l = 1; l < lines.length; l++) {
    ...
    const materia = cols[iMateria >= 0 ? iMateria : 1]?.trim();
    const assunto = cols[iAssunto >= 0 ? iAssunto : 2]?.trim();
    if (!materia && !assunto) continue;

    let gabarito: string | undefined;
    if (incluirGabarito && iGabarito >= 0) {
      const letra = cols[iGabarito]?.toUpperCase().slice(0, 1);
      if (letra && /^[A-E]$/.test(letra)) gabarito = letra;
    }

    rows.push({
      numero,
      areaBloco: iArea >= 0 ? cols[iArea] : undefined,
