...
const PADROES_SOCIOLOGIA: RegExp[] = [
  ...
  /\bcancao\b.*\b(critica|sociedade|contemporanea)\b/,
  /\bletra\s+de\s+(musica|canção)\b.*\b(critica|sociedade)\b/,
];
...
const PADROES_GEOGRAFIA_AMBIENTAL: RegExp[] = [
  ...
  /\bcredito\s+de\s+carbono\b/,
  /\bmercado\s+de\s+carbono\b/,
  /\bcompensacao\s+de\s+emissoes\b/,
  /\bpolitica\s+climatica\b/,
  /\bareas?\s+(urbanas?\s+)?de\s+risco\b/,
  /\bdeslizamento\b/,
...
/** Pós-validação linguagens — corrige conflitos evidentes após IA. */
export function validarRoteamentoLinguagensPosIA(
  texto: string,
...
export function validarRoteamentoHumanasPosIA(texto: string, rota: RotaMinima): RotaMinima {
  return aplicarRoteamentoDeterministicoHumanas(texto, rota);
}
