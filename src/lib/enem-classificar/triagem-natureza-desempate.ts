...
const REGRAS_PRIORIDADE: RegraDesempate[] = [
  {
    ...
    materia: "Biologia",
    quando:
      /\b(sucessao\s+ecologica|perturbac(o|ao)es?\s+ambientais|apos\s+a\s+perturbacao|recuperacao\s+ecologica|comunidade\s+(pioneira|climax))\b/,
  },
  {
  ...
  },
  {
    ...
    materia: "Física",
    quando:
      /\b(colis(ao|oes)\s+(frontais?|sucessivas?)|conservacao\s+da\s+quantidade\s+de\s+movimento|quantidade\s+de\s+movimento|momento\s+linear|velocidade\s+apos\s+a\s+colisao)\b/,
  },
];
