...
import { extrairProvaOrquestrada } from "@/lib/prova-extracao-orquestrador";
import {
...
/** Extração híbrida: n8n (fast path) → Pipeline V2 (fallback). */
export async function POST(
  ...
  try {
    const resultado = await extrairProvaOrquestrada(
      pdfBuffer,
      pdfNome,
      provaId,
      {
    ...
    if (resultado.rows.length === 0) {
      return NextResponse.json(
        {
          error:
            "Nenhuma questão extraída. Confira o PDF, o total de questões no cadastro e tente novamente.",
    ...
    return NextResponse.json({
      ...
      totalEsperado: prova.totalQuestoes,
      modeloUsado: resultado.pipeline?.modeloUsado,
      metricasN8n: resultado.metricasN8n,
      motivoFallback: resultado.motivoFallback,
      avisos: resultado.avisos,
      etapas: resultado.etapas,
      rows: aplicar ? undefined : resultado.rows,
      csv: resultado.pipeline?.csv || undefined,
      ...relatorioResposta,
    });
  } catch (e) {
    const message = e instanceof Error ? e.message : "Erro na extração híbrida";
    console.error(e);
