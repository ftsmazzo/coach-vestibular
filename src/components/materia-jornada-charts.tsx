...
/** Matérias que aparecem em pelo menos 2 registros com questões. */
function materiasParaLinha(series: MateriaSerieProva[], candidatos: string[]): string[] {
  const counts = new Map<string, number>();
  for (const s of series) {
    for (const mid of s.materiasPresentes) {
      counts.set(mid, (counts.get(mid) ?? 0) + 1);
    }
  }
  return candidatos.filter((mid) => (counts.get(mid) ?? 0) >= 2).slice(0, 5);
}
...
export function MateriaJornadaCharts({
  ...
  const linhaMaterias = materiasParaLinha(seriesPorProva, materiaIdsOrdenados);

  const lineData = seriesPorProva.map((s) => {
    const row: Record<string, string | number | null> = {
      data: s.dataLabel,
      registro: s.nomeCurto,
      Geral: s.pctGeral,
    };
    for (const mid of linhaMaterias) {
      const label = getMateriaLabel(mid);
      row[label] = s.materias[mid] ?? null;
    }
    return row;
  });
  ...
  return (
    <div className="space-y-8">
      ...
      {lineData.length >= 2 && linhaMaterias.length > 0 && (
        <div>
          <h3 className="mb-1 text-sm font-semibold text-slate-800">
